...
- **pre-commit**: 스테이징된 파일에 lint-staged 실행 (ESLint 자동수정 + Prettier 포맷)
- **pre-push**: 전체 타입 검사 (`tsc --noEmit`) — 타입 오류가 있으면 push 차단
...
이 파일은 `middleware.ts` 대신 `proxy.ts`라는 이름을 사용한다 (Next.js 16.2+ Fluid compute 패턴).
